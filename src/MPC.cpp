...
const double ref_v = 65; //a tribute to max speed limit in NY state
...
class FG_eval {
 public:
  ...
  // The cost is stored at fg[0]. Any addition is to be made at fg[0] and hence 
  // at the start, fg[0] is set to 0. 
  fg[0] = 0;
  ...
  for (int i = 0; i < N - 1; i++){
    ...
    fg[0] += weights[5] * CppAD::pow(vars[delta_start + i] * vars[v_start+i], 2);
  }
  ...
  for (int t = 1; t < N; t++) {
      ...
      //using previous actuations to acccount for latency issues
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs, Eigen::VectorXd weights) {
  ...
  //
  // 4 * 10 + 2 * 9
  float const no_state_vecs = 6;
  float const no_actuators = 2;
  size_t n_vars = N * no_state_vecs + (N - 1) * no_actuators;
  // TODO: Set the number of constraints
  size_t n_constraints = N * no_state_vecs;
  ...
  for (int i = 0; i < delta_start; i++) {
    vars_lowerbound[i] = -99999;
    vars_upperbound[i] = 99999;
  }
  ...
  sol_vector.push_back(solution.x[delta_start]);
  sol_vector.push_back(solution.x[a_start]);

  for (int i = 0; i < N - 1; i++) {
    sol_vector.push_back(solution.x[x_start + i + 1]);
    sol_vector.push_back(solution.x[y_start + i + 1]);
  }
